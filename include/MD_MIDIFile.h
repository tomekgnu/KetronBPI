...
struct MD_MIDIFile{
	void (*_midiHandler)(int fd,midi_event *pev);   ///< callback into user code to process MIDI stream
 ...
 */
   ...
   */
  inline uint32_t getTickTime(struct MD_MIDIFile *m);
   ...
   * \return the tempo.
   */
  inline uint16_t getTempo(struct MD_MIDIFile *m);
   ...
   * \return the tempo.
   */
  inline int16_t getTempoAdjust(struct MD_MIDIFile *m);
   ...
   */
  inline uint16_t getTicksPerQuarterNote(struct MD_MIDIFile *m);
   ...
   */
  inline uint16_t getTimeSignature(struct MD_MIDIFile *m);
   ...
   */
  inline uint8_t getFormat(struct MD_MIDIFile *m);
   ...
   */
  inline uint8_t getTrackCount(struct MD_MIDIFile *m);
  /** @} */
   ...
   * \return No return data.
   */
  inline void looping(struct MD_MIDIFile *m, BOOL bMode);
 ...
 /** 
  ...
  void    initialise(struct MD_MIDIFile *m,int fd);   ///< initialize class variables all in one place
